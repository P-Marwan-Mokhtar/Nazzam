...
تطبيق مهام عربي (RTL) يعمل كـ PWA، بدون أي framework (Vanilla JS). فيه صفحة هبوط (Landing) وتطبيق فعلي في مجلد `app/`، والموقع منشور على Vercel (مربوط بالجيت هب — هيدرات `vercel.json` مطبقة على الإنتاج).
