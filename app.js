(function () {
  ...
  */
  ...
  function parseDurationToMinutes(str) {
    ...
    if (/نص\s*ساعة|نصف\s*ساعة/i.test(text)) {
      totalMinutes += 30;
      matched = true;
    }
    if (/ربع\s*ساعة/i.test(text)) {
      totalMinutes += 15;
      matched = true;
    }
    ...
    if (!matched) {
      ...
      if (plain) {
        totalMinutes = parseFloat(plain[1]) * 60;
        matched = true;
      }
    }
  ...
  function computeWeekStats() {
    ...
    state.keywords.forEach((k) => {
      if (k.filterId) nameToFilterId[k.name] = k.filterId;
    });
    ...
    if (
      todayTasksForStreak.length > 0 &&
      todayTasksForStreak.every((t) => t.done)
    )
      streak++;
    ...
    let bestDay = null,
      bestDayMs = -1;
    weekDays.forEach((date) => {
      if (dayTotals[date] > bestDayMs) {
        bestDayMs = dayTotals[date];
        bestDay = date;
      }
    });
    if (bestDayMs <= 0) bestDay = null;

    const topTasks = Object.entries(taskTimeMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    const freqMap = {};
    ...
    const topFrequent = Object.entries(freqMap)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5);
    ...
    const neglected = state.keywords
      .filter((k) => !recentNames.has(k.name))
      .slice(0, 8);
  ...
  function renderStatsView() {
    ...
    const weekHours = s.weekDays.map(
      (d) => +(s.dayTotals[d] / 3600000).toFixed(2),
    );
    ...
    const topTasksHours = s.topTasks.map(
      ([, ms]) => +(ms / 3600000).toFixed(2),
    );
            ...
            datasets: [
              {
                data: [
                  s.doneCount,
                  Math.max(0, s.totalTaskCount - s.doneCount),
                ],
          ...
          options: {
            ...
            plugins: {
              legend: {
                position: "bottom",
                rtl: true,
                labels: { color: inkColor, font: { size: 11 } },
              },
            },
          },
        }),
          ...
          options: {
            ...
            scales: {
              ...
              y1: {
                ...
                position: "right",
                grid: { display: false },
                ticks: { color: inkColor, callback: (v) => v + "%" },
              },
            },
          },
        }),
  ...
  function mapAuthError(e) {
    ...
    if (
      ...
      return "الإيميل ده مستخدم بالفعل، جرب تسجّل دخول بدل كده";
    if (msg.includes("Email not confirmed"))
      return "لازم تأكد إيميلك الأول، شوف رسالة التأكيد في بريدك";
    ...
    if (
      ...
      return "محاولات كتير قوي في وقت قصير، استنى شوية وجرب تاني";
    if (msg.toLowerCase().includes("captcha"))
      return "تعذر التحقق الأمني، حدّث الصفحة وجرب تاني";
  ...
  function wirePasswordToggle(inputId, toggleId) {
    ...
    toggle.onclick = () => {
      ...
      toggle.querySelector(".material-icons").textContent = isPass
        ? "visibility_off"
        : "visibility";
    };
  }
  ...
  function renderAccountModal(errorMsg) {
    ...
    const errorHtml = errorMsg
      ? `<div class="account-error">${errorMsg}</div>`
      : "";
    ...
    if (accountModalMode === "forgot") {
      ...
      `;
      document.getElementById("accSwitchMode").onclick = () => {
        accountModalMode = "signin";
        renderAccountModal();
      };
      const submit = () =>
        handleForgotPassword(document.getElementById("accEmail").value.trim());
    ...
    if (accountModalMode === "signin") {
      ...
      document.getElementById("accForgotBtn").onclick = () => {
        accountModalMode = "forgot";
        renderAccountModal();
      };
  ...
  async function signOutUser() {
    if (
      !confirm(
        "متأكد من تسجيل الخروج؟ هتحتاج تسجّل دخول تاني عشان توصل لنفس البيانات.",
      )
    )
      return;
    try {
    ...
    if (timers.length === 0) {
      ...
      timers.forEach((t) => {
        ...
        const remainingMs = isCountdown
          ? Math.max(0, t.targetMs - getElapsedMs(t))
          : getElapsedMs(t);
    ...
    document.querySelectorAll(".custom-select").forEach((sel) => {
      ...
      menu.querySelectorAll(".custom-select-option").forEach((opt) => {
        opt.onclick = (e) => {
          ...
          sel.querySelector(".custom-select-label").textContent =
            opt.textContent;
          menu
  ...
  function render() {
    ...
    const totalActualMinutes = dayTasks.reduce(
      (sum, t) => sum + parseDurationToMinutes(t.actualDuration),
      0,
    );
    const totalHoursText =
      totalActualMinutes > 0
        ? `الوقت الفعلي: ${formatMinutes(totalActualMinutes)}`
        : "";
    ...
    if (bankOpen || closingBank) {
      ...
      const visibleKeywords = searchNormalized
        ? filterMatched.filter((k) =>
            normalizeArabic(k.name).includes(searchNormalized),
          )
        : filterMatched;

      if (visibleKeywords.length === 0) {
        let emptyMsg = "بنك المهام فاضي. ضيف مهامك الأساسية من فوق.";
        if (state.keywords.length > 0 && searchNormalized)
          emptyMsg = "مفيش نتائج مطابقة للبحث.";
        else if (state.keywords.length > 0)
          emptyMsg = "مفيش مهام في الفلتر ده.";
    ...
    if (dayTasks.length === 0) {
      ...
      dayTasks.forEach((t) => {
        ...
        const pct =
          targetMin > 0 ? Math.round((actualMin / targetMin) * 100) : 0;
  ...
  function attachEvents() {
    ...
    contentEl.onclick = async (e) => {
      ...
      if (!btn) {
        ...
        const mainEl = e.target.closest(
          '.task-main[data-action="toggle-task"]',
        );
        if (mainEl) {
      ...
      if (action === "toggle-bank") {
        if (bankOpen) {
          ...
          if (bankCloseTimeoutId) {
            clearTimeout(bankCloseTimeoutId);
            bankCloseTimeoutId = null;
          }
          closingBank = false;
          bankOpen = true;
        ...
        state.days[selectedDate] = state.days[selectedDate].filter(
          (t) => t.id !== id,
        );
        ...
        if (
          confirm(
            "متأكد إنك عايز تحذف الفلتر ده؟ المهام اللي فيه هترجع بدون فلتر (وتفضل ظاهرة في الكل).",
          )
        ) {
          state.filters = state.filters.filter((f) => f.id !== id);
          state.keywords.forEach((k) => {
            if (k.filterId === id) k.filterId = null;
          });
        ...
        if (val) {
          ...
          if (kw) {
            kw.name = val;
            kw.filterId =
              filterSelect && filterSelect.dataset.value
                ? filterSelect.dataset.value
                : null;
          }
        }
    ...
    if (addKeywordBtn && newKeywordInput) {
      const handleAdd = () => {
        ...
        pendingTaskName = val;
        pendingTaskFilterId =
          newKeywordFilter && newKeywordFilter.dataset.value
            ? newKeywordFilter.dataset.value
            : null;
    ...
    if (editInput) {
      editInput.onkeydown = (e) => {
        if (e.key === "Enter")
          document.querySelector('button[data-action="save-keyword"]').click();
        if (e.key === "Escape")
          document
            .querySelector('button[data-action="cancel-keyword"]')
            .click();
      };
    }
  ...
  (async function init() {
    ...
    document.addEventListener("click", (e) => {
      document
        ...
        .forEach((s) => s.classList.remove("open"));
      if (
        openDurationPopoverTaskId &&
        !e.target.closest(".duration-popover") &&
        !e.target.closest(".duration-badge")
      ) {
        hideDurationPopover();
      }
      if (
        openClockChoiceTaskId &&
        !e.target.closest(".clock-choice-popover") &&
    ...
    document.getElementById("choiceTodayBtn").onclick = async () => {
      ...
      if (!state.days[selectedDate]) state.days[selectedDate] = [];
      const exists = state.days[selectedDate].some(
        (t) => t.name === pendingTaskName,
      );
      if (!exists) {
        state.days[selectedDate].push({
          id: uid(),
